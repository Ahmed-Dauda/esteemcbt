...
# ✅ Define country choices here (replace or import from utils if you have)
country_choice = [
    ('Nigeria', 'Nigeria'),
    ('Ghana', 'Ghana'),
    ('Kenya', 'Kenya'),
...
# -------------------------------
# 1️⃣ Simple Signup Form (For general users)
# -------------------------------
class SimpleSignupForm(UserCreationForm):
    first_name = forms.CharField(max_length=12, label='First name')
    last_name = forms.CharField(max_length=225, label='Last name')
    countries = forms.ChoiceField(choices=country_choice, label='Country')
    phone_number = forms.CharField(max_length=225, required=False, widget=forms.HiddenInput())

    class Meta:
        model = NewUser
        fields = ['email', 'first_name', 'last_name', 'countries', 'password1', 'password2']
    ...
    def save(self, commit=True):
        ...
        user.countries = self.cleaned_data['countries']
        if not user.school:
            default_school, _ = School.objects.get_or_create(school_name="Codethinkers Academy")
            user.school = default_school
        if commit:
            user.save()
        return user
...
# -------------------------------
# 2️⃣ School Student Signup Form
# -------------------------------
class SchoolStudentSignupForm(UserCreationForm):
    first_name = forms.CharField(max_length=50, label='First name')
    last_name = forms.CharField(max_length=50, label='Last name')
    referrer_code = forms.CharField(max_length=50, required=False, label='Referral Code')
    school_name = forms.CharField(max_length=225, required=False, label='School Name')
    countries = forms.ChoiceField(choices=country_choice, label='Country')
    ...
    def save(self, commit=True):
        user = super().save(commit=False)
        user.countries = self.cleaned_data['countries']
        ref_code = self.cleaned_data.get('referrer_code')

        # Optional: handle referrer if provided
        if ref_code:
            try:
                referrer = ReferrerMentor.objects.get(code=ref_code)
...
# -------------------------------
# 3️⃣ Teacher Signup Form
# -------------------------------
class TeacherSignupForm(UserCreationForm):
    first_name = forms.CharField(max_length=50, label='First name')
    last_name = forms.CharField(max_length=50, label='Last name')
    countries = forms.ChoiceField(choices=country_choice, label='Country')
    phone_number = forms.CharField(max_length=225, required=False, label='Phone Number')
...
# -------------------------------
# 4️⃣ Course Grade Form (example)
# -------------------------------
class CourseGradeForm(forms.ModelForm):
    class Meta:
        model = CourseGrade
        fields = ['course', 'grade', 'comment']
