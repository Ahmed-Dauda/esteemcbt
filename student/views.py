...
#     # Get distinct sessions, terms, and exam types where the student has results
#     report_cards = Result.objects.filter(student=student).values('session', 'term', 'exam_type__name').distinct()
#     print(report_cards, 'report_cards')
#     context = {
...
#     report_cards = Result.objects.filter(student=student).values('session', 'term').distinct()
...
#         # Get all students' total marks for the subject across CA, MIDTERM, and EXAM
#         all_students_total_marks = Result.objects.filter(
#             exam__course_name=subject,  # Filter by the subject
#             session=session,            # Filter by session
#             term=term,                  # Filter by term
#               # Filter by student class
#         ).values('student').annotate(
#             total_marks=Sum('marks')  # Sum the marks for each student
#         )
...
#         # Find the position of the current student by their ID
#         position = next(
#             (i + 1 for i, marks in enumerate(sorted_marks) if marks['student'] == student.id),
#             None  # In case the student is not found in the sorted list
#         )

#         # Store the position in `subject_positions` for the current subject
#         subject_positions[subject] = position
#         # Debug print to see the positions

#         comment = grade_comments.get(grade, 'No comment available')
#         subject_comments[subject] = comment
#         #     # Get the highest average in the class

#         # Sort students by average marks
#         sorted_students = sorted(all_students_total_marks, key=lambda x: x['total_marks'], reverse=True)
#         # Get the lowest average
#         student_id = student.id  # Use the student's ID directly
#         # Calculate final position based on sorted list
#         final_position2 = next((index + 1 for index, student_info in enumerate(sorted_students) if student_info['student'] == student_id), None)

#       # working now Initialize dictionaries to store high, low, total marks, and counts for each subject
#         # Initialize dictionaries to store high, low, total marks, and counts for each subject
#     subject_highs = {}
#     subject_lows = {}
#     subject_totals = {}
#     subject_counts = {}
...
#     all_students = Profile.objects.filter(user__student_class=student.student_class)
#     for other_student in all_students:
#         # Fetch distinct results for the current student
#         other_results = Result.objects.filter(
...
#             # Fetch CA, MIDTERM, and EXAM marks for the current student and subject
#             ca_marks = Result.objects.filter(exam__course_name=subject, term=term, session=session, student=other_student, exam_type__name='CA').aggregate(Sum('marks'))['marks__sum'] or 0
#             midterm_marks = Result.objects.filter(exam__course_name=subject, term=term, session=session, student=other_student, exam_type__name='MIDTERM').aggregate(Sum('marks'))['marks__sum'] or 0
#             exam_marks = Result.objects.filter(exam__course_name=subject, term=term, session=session, student=other_student, exam_type__name='EXAM').aggregate(Sum('marks'))['marks__sum'] or 0

#             # Get the total possible marks for each exam type
...
#             total_possible_marks_for_subject = ca_total_marks + midterm_total_marks + exam_total_marks

#             # Calculate total marks for the subject
#             if total_possible_marks_for_subject == 0:
#                 total_marks = 0
#             else:
#                 if not midterm_marks and not exam_marks:
#                     total_marks = (ca_marks / ca_total_marks) * 100
...
#                 else:
#                     total_marks = (
#                         (ca_marks / ca_total_marks) * 100 * (ca_total_marks / total_possible_marks_for_subject) +
#                         (midterm_marks / midterm_total_marks) * 100 * (midterm_total_marks / total_possible_marks_for_subject) +
#                         (exam_marks / exam_total_marks) * 100 * (exam_total_marks / total_possible_marks_for_subject)
#                     )
...
#             # print(total_max_marks, 'total_max_marks')
#             # grade = calculate_grade(total_marks, student.user.school)
#             # print(subject_totals, 'subject_totals')
#             # Calculate high, low, and add to totals for each subject
#             if subject not in subject_highs:
#                 # Initialize high, low, and total for this subject
#                 subject_highs[subject] = total_marks
#                 subject_lows[subject] = total_marks
#                 subject_totals[subject] = round(total_marks,0)  # Add this student's marks to the total
#                 # print(subject_totals[subject], 'subject_totals[subject]')
#                 subject_counts[subject] = 1  # Start count for this subject
#             else:
#                 # Update high, low, and total for the subject
#                 subject_highs[subject] = max(subject_highs[subject], total_marks)
#                 subject_lows[subject] = min(subject_lows[subject], total_marks)
#                 subject_totals[subject] += total_marks  # Add this student's marks to the total
#                 subject_counts[subject] += 1  # Increment the count for this subject

#     # Now you have high, low, and total marks for each subject
#     for subject in subject_highs:
#         # Calculate the subject average, only considering valid counts
#         subject_average = subject_totals[subject] / subject_counts[subject] if subject_counts[subject] > 0 else 0
#         sub_class_averages[subject] = subject_average

#         # highest_average_in_class = max(student_averages) if student_averages else 0
#         # lowest_average_in_class = min(student_averages) if student_averages else 0


#         # Store the results with rounded values
#         # grade = calculate_grade(total_marks, student.user.school)
#         subject_grades1[subject] = {
#             # 'grade': grade,  # Assuming `grade` is calculated elsewhere
#             'low': round(subject_lows[subject], 1),
#             'high': round(subject_highs[subject], 1),
#             'average': round(subject_average, 1)
#         }
    ...
    for result in results:
        ...
        for result in results:
            ...
            for student_marks in students_total_marks:
                ...
                ca_total_marks = Course.objects.filter(schools = request.user.school,term=term, session=session, exam_type__name='CA').values('total_marks').first()
                ca_total_marks = ca_total_marks['total_marks'] if ca_total_marks else 0

                midterm_total_marks = Course.objects.filter(schools = request.user.school,term=term, session=session, exam_type__name='MIDTERM').values('total_marks').first()
                midterm_total_marks = midterm_total_marks['total_marks'] if midterm_total_marks else 0

                exam_total_marks = Course.objects.filter(schools = request.user.school,term=term, session=session, exam_type__name='EXAM').values('total_marks').first()
                exam_total_marks = exam_total_marks['total_marks'] if exam_total_marks else 0
...
#         .values('school__school_name', 'month', 'year')
#         .annotate(total_downloads=Sum('download_count'))
#         .order_by('year', 'month', 'school__school_name')
#     )
...
# # real view
# @login_required
# def generate_report_card(request, session, term, exam_type):
...
#         session=session,
#         term=term,
#         exam_type__name=exam_type  # Filter by exam type
#     )
...
#     total_results = Result.objects.filter(result_class=student.student_class, session=session, term=term, exam_type__name=exam_type)
#     students_total_marks = sum(total_result.marks for total_result in total_results)
#     class_average = students_total_marks / student_class_count if student_class_count > 0 else 0
    
#     # Fetch total marks obtained by each student in the class and calculate the highest and lowest averages
#     student_averages = Result.objects.filter(
#         result_class=student.student_class, session=session, term=term, exam_type__name=exam_type
#     ).values('student').annotate(average_marks=Avg('marks'))
...
#     for result in results:
#         subject = result.exam.course_name
#         all_marks = Result.objects.filter(exam__course_name=subject, student__user__student_class=student_class, exam_type__name=exam_type).values_list('marks', flat=True)
...
#         low = Result.objects.filter(exam__course_name=subject, student__user__student_class=student_class, exam_type__name=exam_type).aggregate(Min('marks'))['marks__min']
#         high = Result.objects.filter(exam__course_name=subject, student__user__student_class=student_class, exam_type__name=exam_type).aggregate(Max('marks'))['marks__max']
#         subject_grades[subject] = {'low': low, 'high': high}
...
#     all_students_total_marks = Result.objects.filter(student__user__student_class=student_class, exam_type__name=exam_type).values('student').annotate(total_marks=Sum('marks')).order_by('-total_marks')
...
#     # Calculate the highest and lowest averages among all subjects
#     student_highest_average = max(class_averages.values(), default=0)
#     student_highest_average = round(student_highest_average, 1)
...
#             student__user__student_class=student_class,
#             exam_type__name=exam_type
#         ).aggregate(Min('marks'))['marks__min']
#         if lowest_marks is not None:
#             subject_low_averages.append(lowest_marks)
...
#         'final_position': final_position,  # Add final position to context
#         'student_highest_average': student_highest_average,  # Add highest average to context
#         'student_lowest_average': student_lowest_average,  # Add lowest average to context
#         'overall_lowest_average': overall_lowest_average,  # Add overall lowest average to context
#     }
...
#     student = Profile.objects.select_related('user', 'user__school').get(user=request.user)
#     newusers = NewUser.objects.get(id=request.user.id)
#     # print(newusers.student_class, 'tt')
    
#     # Fetch results related to the student and the associated exam details
#     results = Result.objects.filter(student = student,result_class=student.student_class,session=session, term=term)
#     if not results.exists():
#         return render(request, 'student/dashboard/report_card.html', {'message': 'No results found for this session and term.'})
...
#     class_average = students_total_marks / student_class_count if student_class_count > 0 else 0
#     # Print the class average for debugging
#     # # Calculate the highest and lowest total marks
#     # highest_marks = total_results.aggregate(models.Max('marks'))['marks__max']
#     # lowest_marks = total_results.aggregate(models.Min('marks'))['marks__min']

#     # # Handle cases where no results are found
#     # highest_marks = highest_marks if highest_marks is not None else 0
#     # lowest_marks = lowest_marks if lowest_marks is not None else 0

#     from django.db.models import Sum, Avg

#     # Fetch total marks obtained by each student in the class
#     student_averages = Result.objects.filter(
#         result_class=student.student_class, session=session, term=term
#     ).values('student').annotate(average_marks=Avg('marks'))
#     # print(student_averages, 'student_averages')
#     # Calculate the highest and lowest averages
#     highest_average_in_class = student_averages.aggregate(models.Max('average_marks'))['average_marks__max']
#     lowest_average_in_class = student_averages.aggregate(models.Min('average_marks'))['average_marks__min']
...
#     # Print for debugging
#     print(f'Highest Average in Class1: {highest_average_in_class}')
#     print(f'Lowest Average in Class1: {lowest_average_in_class}')


#     # print(students_total_marks, 'total_marks_students')
